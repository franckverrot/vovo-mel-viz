...
MelViz is a macOS spectogram visualizer that shows the actual `[T, 100]` log-mel a TTS model produces.  It lets you re-runs the vocoder so you can *hear* the edit, and A/Bs it against the original.  It's all live, and can work mid-sentence.  It was built to help debug [Vovo,](https://github.com/franckverrot/vovo) a TTS voice model made from-scratch in Swift with Metal kernels, as a tool to find out *where* the model didn't perform properly.
...
- **View**: a Metal heatmap on a fixed colormap scale (so level errors don't hide behind auto-scaling), and a SceneKit 3-D terrain you can rotate/zoom into.
